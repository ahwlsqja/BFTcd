...
──────────────                     ──────────────
┌─────────────┐                    ┌─────────────┐
│ etcd API    │                    │ etcd API    │  ← 동일 (호환성)
├─────────────┤                    ├─────────────┤
│ MVCC/Watch  │                    │ MVCC/Watch  │  ← 동일
├─────────────┤                    ├─────────────┤
│ Raft (CFT)  │       →            │ HotStuff2   │  ← 교체 (BFT)
├─────────────┤                    │ (BFT)       │
│ Storage     │                    ├─────────────┤
└─────────────┘                    │ Storage     │
                                   └─────────────┘
...
│    │   │              HotStuff2 Engine                 │     │     │
│    │   │                                               │     │     │
│    │   │   • Safety Module: 투표 규칙, 잠금 관리       │     │     │
│    │   │   • Pacemaker: 뷰 동기화, 타임아웃 관리       │     │     │
│    │   │   • Crypto: BLS 서명, QC 생성/검증           │     │     │
│    │   │                                               │     │     │
│    │   └──────────────────────────────────────────────┘     │     │
│    │                                                         │     │
│    └─────────────────────────┬───────────────────────────────┘     │
...
│    │              저장 계층 (Storage Layer)                  │     │
│    │                                                         │     │
│    │   • WAL (Write-Ahead Log): 합의 로그 영속화            │     │
│    │   • State DB: 키-값 상태 저장                          │     │
│    │   • Snapshot: 상태 스냅샷 및 복구                      │     │
│    │                                                         │     │
│    └────────────────────────────────────────────────────────┘     │
│                                                                    │
└───────────────────────────────────────────────────────────────────┘
...
━━━━━━━━━━━━━━
   Leader ─────[블록 제안]─────► 모든 노드
                                    ...
                                    │ 검증 후 투표
                                    ▼
   Leader ◄────[투표 수집]───── 2f+1 노드
        │
        │ 투표 집계 → QC(Quorum Certificate) 생성
        ▼

2단계: COMMIT  
━━━━━━━━━━━━━━
   Leader ─────[QC 포함 다음 블록]────► 모든 노드
                                           │
                                           │ QC 검증 → 커밋
                                           ▼
                                      상태 적용 완료
