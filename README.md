...
2025년 12월 기준, 글로벌 디지털 자산 시장은 단순한 가치 저장을 넘어 '통합 원장(Unified Ledger)'과 '예금 토큰(Deposit Token)'을 통한 기관 간 결제의 시대로 진입했다. 프로젝트 아고라(Project Agorá)와 같은 이니셔티브는 상호 신뢰하지 않는 다수의 금융 기관이 단일 인프라를 공유할 것을 요구한다. 그러나 현재의 사실상 표준인 Kubernetes 기반 인프라는 치명적인 불일치를 안고 있다. 애플리케이션 계층은 BFT를 지향하나, 인프라의 상태를 저장하는 핵심 엔진인 etcd는 여전히 CFT(Crash Fault Tolerance)인 Raft 알고리즘에 의존한다. 본 문서는 이러한 '신뢰의 격차(Trust Gap)'를 해결하기 위해, HotStuff 2.0 합의 알고리즘을 적용한 새로운 상태 저장소 BFTcd를 제안한다. 이는 금융 컨소시엄 인프라에 수학적으로 증명 가능한 무결성과 안전성을 제공한다.

---

# 1. 시장 배경: 디지털 화폐의 새로운 패러다임 (Market Background: The New Paradigm of Digital Currency)
지난 10년간 블록체인 기술이 '탈중앙화된 화폐'라는 실험적 가능성을 증명했다면, 2025년 현재의 금융 시장은 **'프로그래밍 가능한 제도권 금융(Programmable Institutional Finance)'**으로의 거대한 전환을 맞이하고 있다. 이는 단순한 자산의 디지털화(Digitization)를 넘어, 화폐의 성격과 금융 인프라의 구조 자체가 재정의되는 역사적 변곡점이다.


## 1.1 스테이블코인 시장 구조적 성숙과 한계 (Structural Maturation and Limitations of the Stablecoin Market)
...
## 1.3 핵심 트렌드: 컨소시엄의 필연성 (Key Trend: The Inevitability of Consortium Infrastructure) 

예금 토큰과 통합 원장의 등장은 금융 인프라 아키텍처에 있어 **'단일 중앙 집중형 원장'에서 '다자간 분산 원장(DLT)'으로의 이동**을 강제한다. 이 변화는 선택이 아닌 기술적, 경제적 필연이다.
...
**결론적으로, 2025년의 금융은 '은행들의 연합체(Consortium)'가 운영하는 거대한 분산 컴퓨터 위에서 작동한다.** 그러나 이 거대한 시스템은 애플리케이션 계층에서만 BFT(비잔틴 장애 허용)를 달성했을 뿐, 그 하부의 **인프라 조정 계층(Infrastructure Orchestration Layer)** 은 여전히 과거의 신뢰 모델(CFT)에 머물러 있다는 위험한 불균형 속에 놓여 있다. 이것이 바로 우리가 **BFTcd**를 제안하는 이유다.

---

# 2. 문제 정의: 컨소시엄 인프라의 취약점
...
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │               Protected Zone (접근 제어)                     │    │
│  │                                                              │    │
│  │   • K8s API Server: RBAC 기반 권한 관리                     │    │
│  │   • 네트워크: mTLS, 방화벽                                   │    │
│  │   • 감사 로그: 모든 접근 기록                                │    │
│  │                                                              │    │
│  └─────────────────────────────────────────────────────────────┘    │
...
BFTcd는 기술적 합의뿐 아니라 조직적 거버넌스도 지원한다:
...
│   │                   Governance Council                         │   │
│   │                                                              │   │
│   │   Bank A    Bank B    Bank C    Bank D    (Regulator)       │   │
│   │    1표       1표       1표       1표       (옵저버)          │   │
│   │                                                              │   │
│   └─────────────────────────────────────────────────────────────┘   │
│                              │                                       │
...
100 ┤                ╱
    │              ╱
  0 ┼──────────────┴─────────────────────────────────────────
    4     7    10    13    16    19    22  Nodes

→ 노드가 많아질수록 HotStuff2의 효율성이 극대화됨
```
...
|--------|---------|----------|--------------|-------------|
| **etcd (Raft)** | ❌ | ✅ 완벽 | ✅ 매우 성숙 | ⚠️ CFT만 |
| **CockroachDB** | ❌ | ⚠️ 부분 | ✅ 성숙 | ⚠️ CFT만 |
| **Tendermint** | ✅ | ❌ | ✅ 성숙 | ⚠️ K8s 비호환 |
| **Cosmos SDK** | ✅ | ❌ | ✅ 성숙 | ⚠️ K8s 비호환 |
| **BFTcd** | ✅ | ✅ 완벽 | 🔄 개발중 | ✅ 최적 |
